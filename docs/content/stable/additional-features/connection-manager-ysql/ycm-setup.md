...
### Connection Manager SSL states

The YSQL Connection Manager operates in one of two states based on your cluster configuration. This state determines how the manager intercepts and validates incoming client requests.

#### OD_CONFIG_TLS_DISABLE (Cluster TLS is OFF)

Use this mode when the cluster does not have TLS enabled. The default HBA configuration allows unencrypted connections:
...
| SSL mode | Description | Same as PostgreSQL? | Authentication followed? | HBA customization changes behavior? |
| --- | --- | --- | --- | --- |
| disable | Client attempts a non-SSL connection | Yes. Unencrypted connection is established. | Yes | No |
| allow | Tries unencrypted first, then secure (encrypted) | Yes. Unencrypted connection is established. | Yes (first non-SSL attempt succeeds) | No |
| prefer (default in PostgreSQL) | Tries secure first, then unencrypted | Yes. Unencrypted connection is established. | Partially. First SSL attempt is rejected by connection manager, second non-SSL attempt follows connection manager authentication. | No |
| require | Uses secure connection, fails if not available | Yes. If connection fails, server sends 'N' (not supported); client reports: "server does not support SSL, but SSL was required" | No. Client's SSL attempt is rejected by connection manager | No |
| verify-ca | Behaves like require. Additionally, verifies server cert against CA, or fails if no valid matching CA certificates are found. | Yes. If connection fails, server sends 'N' (not supported); client reports: "server does not support SSL, but SSL was required" | No. Client's SSL attempt is rejected by connection manager | No |
| verify-full | Behaves like verify-ca. Additionally, verifies that the server cert matches the host. | Yes. If connection fails, server sends 'N' (not supported); client reports: "server does not support SSL, but SSL was required" | No. Client's SSL attempt is rejected by connection manager | No |

#### OD_CONFIG_TLS_REQUIRE (Cluster TLS is ON)

When TLS is enabled in the cluster, the Connection Manager automatically switches to require mode. It will intercept and reject any non-SSL attempts before checking the HBA rules.
...
When using `sslmode=verify-full`, the security handshake happens at the client level:

- Client-side validation: The client if the server's certificate is signed by a trusted CA.
- Identity match: The client ensures the certificate's hostname matches the actual server address.
- Connection failure on mismatch: If verification fails, the connection is rejected (for example, JDBC and ysqlsh perform this verification).

### Connection Manager SSL enforcement

When using the YSQL Connection Manager, SSL/TLS is enforced in a two-tier hierarchy. The Connection Manager acts as a gateway that intercepts traffic before it reaches the YB-TServer.

1. Proxy layer (Connection Manager): If the cluster has TLS enabled, the Connection Manager enforces SSL immediately. It rejects unencrypted requests (such as `sslmode=disable`) at the entry point.

1. Database layer (YB-TServer): The standard HBA rules are evaluated only after a connection successfully passes the proxy's TLS requirement.

Because the proxy layer takes precedence, a connection attempt may be rejected by the Connection Manager even if the YB-TServer HBA rules (such as trust) would otherwise allow it.

For details and troubleshooting, see [SSL behaviour](../ycm-troubleshoot/#ssl-behaviour) in Troubleshoot.
