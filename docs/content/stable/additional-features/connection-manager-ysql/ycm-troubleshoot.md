...
- **HBA auth method `cert`.** Connection Manager does not support the **server-side** HBA auth method `cert`, where the server authenticates the user via the client's certificate (CN/DN mapping). Client sslmodes **verify-ca** and **verify-full** (where the *client* verifies the *server's* certificate) are supported when connecting through Connection Manager. The unsupported part is only server-side cert-based authentication (the HBA `cert` method). Supporting it would require forwarding client certificates to the database over Unix socket backend connections, which is not implemented.
