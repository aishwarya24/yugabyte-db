...
- How to safely keep track of replication progress. Replication origins allow you to track replay progress in a crash-safe manner without the overhead and database bloat of naive solutions like updating a row in a table for every replayed transaction.

- How to change replication behavior based on the origin of a row. This is particularly useful for preventing loops in bi-directional replication setups, where changes replicated from a remote node should not be replicated back to that same node.
...
### Creating and managing replication origins

Replication origins can be created, dropped, and queried using the following functions:
...
Replication origins can be viewed in the `pg_replication_origin` system catalog:
...
1. This ensures that changes only flow in one direction and prevents replication loops.

For more information on replication origins in PostgreSQL, refer to [Replication Progress Tracking](https://www.postgresql.org/docs/current/replication-origins.html) in the PostgreSQL documentation.
